...
- **Unit Testing & Spec Files:** Whenever creating or updating business logic in services or controllers (e.g., `comments.service.ts`), you **MUST** create or update the corresponding `.spec.ts` unit test file (e.g., `comments.service.spec.ts`) to keep mock DB queries, arguments, and assertions synchronized. Run `nx test api` to verify changes.
