...
- **Unit Testing & Spec Files:** Whenever creating or modifying any services, data services, or components (e.g., `comments.service.ts`, `comments-data.service.ts`, `comment.ts`), you **MUST ALWAYS** provide the updated `.spec.ts` unit test files alongside the code changes to ensure state mutations, signals, template bindings, and mock responses match the implementation. Run `nx test web` to verify changes.
...
- **Strict Typing:** Avoid using `any` for variables, function signatures, or mock objects. Always use explicit TypeScript types, generics, or proper test utilities (e.g., `Mock` from `vitest`).
